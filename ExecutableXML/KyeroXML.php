...
class KyeroXML {
    ...
    public function __construct(string $foldername = 'KyeroXML', string $filename = 'properties.json') {
    ...
    private function processProperty(SimpleXMLElement $property): ?array {
        ...
        // $ownedBy = "Nils Ott Group Ltd.";
        // $contactPerson = "Nils Birger Ott ; Milena Krasteva";
        ...
        try {
           ...
           // Extract country and convert if it's a code
            ...
            if($bedrooms > 0) {
                ...
                $title = "{$bedrooms} Bedroom" . ($bedrooms > 1 ? 's' : '') . " {$property} in " . ($city ?: $state);
            } else {
                $property = $property_type[0];
                $title = "{$property} in " . ($city ?: $state);
            }
    ...
    private function extractSize(SimpleXMLElement $property): int {
        // Try to get built size from surface_area element
        ...
        // If surface_area->built not found, try the standard field mappings
