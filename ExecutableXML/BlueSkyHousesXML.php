...
class BlueSkyHousesXML {
    ...
    public function __construct(string $foldername = 'BlueSkyHousesXML', string $filename = 'properties.json') {
    ...
    private function processProperty(SimpleXMLElement $property): ?array {
        ...
        try {
            $url = $this->getXmlValue($property, $this->config['field_mappings']['website']);
            ...
            $res = $this->apiSender->getPropertyById($fullListingId);
            if ($res['success']) {
                echo "❌ Property already exists in the database\n Update in progress....";
            }
            ...
            $currency = (string) $property->price['currency'] ?? 'EUR';
            ...
            $year_built = $this->getXmlValue($property, $this->config['field_mappings']['year_built']);
            ...
            $country = $this->getXmlValue($property, $this->config['field_mappings']['country']);
            ...
            $title = "{$property_type[0]} {$property_status[0]} in {$city}, {$area}";
    ...
    private function extractDescription(SimpleXMLElement $property, string $lang = 'en'): string {
        ...
        if (isset($property->description)) {
            ...
            if (isset($property->description->$lang)) {
                return trim((string)$property->description->$lang);
            }
            ...
            foreach ($availableLanguages as $fallbackLang) {
                if (isset($property->description->$fallbackLang)) {
                    return trim((string)$property->description->$fallbackLang);
                }
            }
        }
    ...
    public function extractPropertyTypes($property): array {
        ...
        foreach ($typeFields as $field) {
            ...
            if (!empty($typeValue)) {
                // Check if exact type is "Residential Land" (case-insensitive)
                if (strcasecmp(trim($typeValue), 'Residential Land') === 0) {
                    echo "❌ Skipping property Residential Land type \n";
                    return []; // Return empty array to exclude this property
                }
    ...
    private function extractPropertyStatus($property): array {
        $statusFields = $this->config['field_mappings']['property_status'];
        $statuses = [];
        
        foreach ($statusFields as $field) {
            // Handle both SimpleXMLElement and array
            if (is_array($property)) {
                $statusValue = $property[$field] ?? '';
            } else {
            ...
            if (!empty($statusValue)) {
                ...
                if ($allowedStatus) {
                    // Convert "sale" or "Sale" to "For Sale"
                    if (strtolower($statusValue) == 'sale') {
